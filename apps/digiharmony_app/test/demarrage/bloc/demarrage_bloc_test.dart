...
class _MockBienvenueCubit extends Mock implements BienvenueCubit {}

void main() {
  late _MockAppDatabase database;
  late _MockBienvenueCubit bienvenue;
  ...
  setUp(() {
    ...
    when(bienvenue.estBienvenueVue).thenReturn(false);
  });

  DemarrageBloc build() =>
      DemarrageBloc(database: database, bienvenueCubit: bienvenue);
  ...
  blocTest<DemarrageBloc, DemarrageState>(
    ...
    build: () {
      when(bienvenue.estBienvenueVue).thenReturn(true);
      return build();
    },
    act: (b) => b.add(const DemarrageDemarre(dureeMinimale: court)),
    wait: const Duration(milliseconds: 60),
